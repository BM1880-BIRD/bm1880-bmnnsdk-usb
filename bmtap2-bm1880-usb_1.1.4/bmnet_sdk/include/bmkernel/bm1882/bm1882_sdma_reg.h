...
#define SDMA_DESC_REG_BYTES (0x40)
#define SDMA_ENGINE_DESCRIPTOR_NUM  (SDMA_DESC_REG_BYTES >> 2)
#define SDMA_NUM_BASE_REGS (0x8)
...
static inline void parse_sdma_reg(sdma_reg_t *r, const u32 *p)
{
  r->vld = p[0] & 1;
  ...
  r->src_fmt = (p[1] >> 7) & 1;
  r->sys_dtype = (p[1] >> 8) & 1;
  r->rsv3_2 = (p[1] >> 9) & ((1u << 4) - 1);
  r->rsv3_3 = (p[1] >> 13) & 1;
  r->rsv3_4 = (p[1] >> 14) & 1;
  r->rsv4_1 = (p[1] >> 15) & 1;
  r->wait_id_tdma0 = (p[1] >> 16) & ((1u << 16) - 1);
  r->wait_id_tdma1 = p[2] & ((1u << 16) - 1);
  r->wait_id_eng3 = (p[2] >> 16) & ((1u << 16) - 1);
...
static inline void emit_sdma_reg(const sdma_reg_t *r, u32 *_p)
{
  ...
  p[3] = (r->const_val & ((1u << 8) - 1)) |
         ...
         ((r->rsv4_4 & 1) << 23) |
         ((r->rsv4_5 & ((1u << 8) - 1)) << 24);
  p[2] = (r->wait_id_tdma1 & ((1u << 16) - 1)) |
         ((r->wait_id_eng3 & ((1u << 16) - 1)) << 16);
...
static inline void reset_sdma_reg(sdma_reg_t *r)
{
  r->vld = 0x0;
  r->compress_en = 0x0;
  r->eod = 0x0;
...
static inline void trace_sdma_reg(sdma_reg_t *r, const char *tag)
{
#define trace_one_reg(name) \
  ...
  trace_one_reg(rsv3_2);
  trace_one_reg(rsv3_3);
  trace_one_reg(rsv3_4);
  trace_one_reg(rsv4_1);
  trace_one_reg(wait_id_tdma0);
  trace_one_reg(wait_id_tdma1);
  trace_one_reg(wait_id_eng3);
