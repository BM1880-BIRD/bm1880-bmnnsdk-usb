...
public:
  ...
  CpuOp(): CpuLayer(), op_(), global_input_(), global_output_(), ctx_() {}
  ...
  void get_op_coeffs(std::vector<std::pair<gaddr_t, uint64_t> >& info);

  void relocate_op_coeffs(std::vector<gaddr_t>& addrs);

  void func(int id, void* p);

protected:
  TensorOp op_;
  std::vector<gaddr_t> global_input_;
  std::vector<gaddr_t> global_output_;
  std::vector<gaddr_t> global_weight_;
  bmctx_t ctx_;
