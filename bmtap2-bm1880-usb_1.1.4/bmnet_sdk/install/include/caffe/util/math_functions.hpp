...
#endif  // !CPU_ONLY
int compute_rshift(float& threshold_y, std::vector<float>& threshold_x_vector);
int compute_rshift_gpu(float& threshold_y, std::vector<float>& threshold_x_vector);

#ifdef USE_GPU_INT8_INFERENCE
template <typename Dtype>
void SaturateInt8_gpu(Blob<Dtype> *bottom);

template <typename Dtype>
void SaturateUint8_gpu(Blob<Dtype> *bottom);

template <typename Dtype>
void QuantizeInt8_gpu(Blob<Dtype> *bottom,  bool uint_flag);

template <typename Dtype>
void DequantizeInt8_gpu(Blob<Dtype> *bottom,bool uint_flag);

template <typename Dtype>
void RightShift_gpu(Blob<Dtype>*top);

template <typename Dtype>
void SaturateInt16_gpu(Blob<Dtype>*top);
