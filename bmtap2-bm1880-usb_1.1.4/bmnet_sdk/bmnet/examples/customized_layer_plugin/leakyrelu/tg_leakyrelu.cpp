...
#include <bmnet/targets/plat-bm188x/BM1880BackendContext.hpp>

namespace bmnet {

void TGLeakyReluPluginFixedInst::dump() {
...
void TGLeakyReluPluginFixedInst::compute() {
#if 0
  const TGCustomizedParameter& param = op_.tg_customized_param();
  ...
  add_cpu_op(ctx_->bm_get_bmk(),"LeakyRelu", op_);
#else
  const TGCustomizedParameter& param = op_.tg_customized_param();
  const TensorShape& input_shape = op_.input_shape(0);
  float negative_slope = param.f32_param(0);
...
void TGLeakyReluPluginFixedInst::forward(
    ...
    int                   input_w)
{
  ...
  int slice_num                 = get_slice_num_element_wise(*ctx_, 3, count + 1);

  auto *ctx_1880 = dynamic_cast<BM1880BackendContext *>(ctx_);
  ...
  for (int slice_idx = 0; slice_idx < slice_num; slice_idx++) {
      ...
      tensor_lmem *bottom = ctx_1880->tl_alloc(input_shape, FMT_I8, CTRL_AL);
      tensor_lmem *relu = ctx_1880->tl_alloc(input_shape, FMT_I8, CTRL_AL);
      tensor_lmem *neg = ctx_1880->tl_alloc(input_shape, FMT_I8, CTRL_AL);

      // load
      ctx_1880->gdma_load(bottom, slice_bottom_gaddr, CTRL_NEURON);
      // 0. relu = relu(bottom)
      ...
      ctx_1880->tpu_mul_const(&p);
      bmk1880_min_const_param_t p1;
      p1.min = neg;
      p1.a = bottom;
      p1.b = 0;
      p1.b_is_signed = 1;
      ctx_1880->tpu_min_const(&p1);
      p.res_high = NULL;
      p.res_low = neg;
      p.a = neg;
      p.b = le_scale;
      ...
      p2.a = relu;
      p2.b = neg;
      ctx_1880->tpu_or_int8(&p2);
      //move result to global
      ctx_1880->gdma_store(bottom, slice_top_gaddr, CTRL_NEURON);
      //free
      ctx_1880->tl_free(neg);
      ctx_1880->tl_free(relu);
      ctx_1880->tl_free(bottom);
