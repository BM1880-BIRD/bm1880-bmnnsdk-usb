...
inline Tensor Tensor::_th_clamp(Scalar min, Scalar max) const {
    return type()._th_clamp(*this, min, max);
}
inline Tensor Tensor::_th_clamp_min(Scalar min) const {
    return type()._th_clamp_min(*this, min);
}
inline Tensor Tensor::_th_clamp_max(Scalar max) const {
    return type()._th_clamp_max(*this, max);
}
...
inline std::tuple<Tensor,Tensor> Tensor::_fused_dropout(double p, Generator * generator) const {
    return type()._fused_dropout(*this, p, generator);
}
inline Tensor Tensor::_masked_scale(const Tensor & mask, double scale) const {
    return type()._masked_scale(*this, mask, scale);
}
